...
# Helper function to get full visual path
...
t.setup(600, 800)
t.bgcolor("white")
t.bgpic(get_path("background.gif"))
t.tracer(0)  # Turn off automatic screen updates

shapes = ["froggy.gif", "car1.gif", "car2.gif", "car3.gif", "log1.gif", "turtles2.gif",  
          "bike.gif", "turtle.gif", "turtles3.gif", "bus.gif", "home.gif", "goal.gif",
          "tree1.gif", "small_froggy.gif", "lives.gif", "one.gif", "two.gif", 
          "three.gif", "time.gif", "tree2.gif", "30sec.gif", "40sec.gif"]

# Register images with a single loop using full directory paths
for shape in shapes:
    t.register_shape(get_path(shape))
...
player = Player(0, -300, 50, 50, get_path("froggy.gif")) # Player instance
player.define(pen) # Renders the frog at its default starting position 


level_1 = [
    Car(0, -135, 65, 20, get_path("car1.gif"), 2), 
    Car(0, -215, 50, 20, get_path("car3.gif"), -1.5), 
    Car(0, -255, 50, 20, get_path("car2.gif"), 1.7), 
    Car(0, -95, 75, 20, get_path("bus.gif"), -1.4),
    Car(-320, -255, 50, 20, get_path("car2.gif"), 1.7), 
    Log(-350, 95, 105, 20, get_path("log1.gif"), -1.3),
    Car(0, -175, 40, 20, get_path("bike.gif"), 1.9), 
    Log(0, 55, 105, 20, get_path("log1.gif"), 1.1),
    Log(0, 95, 105, 20, get_path("log1.gif"), -1.3), 
    Log(0, 135, 105, 20, get_path("log1.gif"), 1.2),
    Turtles(0, 175, 10, 20, get_path("turtle.gif"), -2), 
    Turtles(0, 255, 60, 20, get_path("turtles2.gif"), -1.8),
    Turtles(0, 215, 110, 20, get_path("turtles3.gif"), 1.5), 
    Goal(80, 0, 30, 20, get_path("tree1.gif")),
    Goal(-215, 10, 70, 40, get_path("tree2.gif")), 
    Turtles(-300, 175, 10, 20, get_path("turtle.gif"), -2),
    Turtles(-280, 255, 60, 20, get_path("turtles2.gif"), -1.8), 
    Turtles(-250, 215, 110, 20, get_path("turtles3.gif"), 1.5),
    Log(-350, 55, 105, 20, get_path("log1.gif"), 1.1)
]

homes = [
    Goal(0, 315, 50, 20, get_path("home.gif")), 
    Goal(-100, 315, 50, 20, get_path("home.gif")),
    Goal(-200, 315, 50, 20, get_path("home.gif")), 
    Goal(100, 315, 50, 20, get_path("home.gif")),
    Goal(200, 315, 50, 20, get_path("home.gif"))
]
...
while True:
    ...
    pen.goto(-200, -325)
    pen.shape(get_path("small_froggy.gif"))
    ...
    pen.goto(-250, 375)
    pen.shape(get_path("lives.gif"))
    pen.stamp()
    pen.goto(-205, 373)
    
    if player.lives == 3:
        pen.shape(get_path("three.gif"))
        pen.stamp()
    elif player.lives == 2:
        pen.shape(get_path("two.gif"))
        pen.stamp()
    elif player.lives == 1:
        pen.shape(get_path("one.gif"))
        pen.stamp()
    else:
        pen.shape(get_path("lives.gif"))
        pen.stamp()
    ...
    for obj in objects:
        if player.iscollision(obj):
            if isinstance(obj, Car):
                ...
                if obj in homes:
                    # Fixed string equality check
                    if obj.image == get_path("goal.gif"):
                        player.y -= 40
                    else:
                        player.home()
                        obj.image = get_path("goal.gif")
                        player.frogs_home += 1
                elif obj in level_1:
                    if player.x < obj.x:
                        player.x -= 40
    ...
    if player.y > 40 and not player.collision:
        player.lives -= 1
        player.home()

    if player.frogs_home < 3:
        pen.goto(255, 370)
        pen.shape(get_path("40sec.gif"))
        pen.stamp()
    else:
        pen.goto(255, 370)
        pen.shape(get_path("30sec.gif"))
        pen.stamp()
    ...
    pen.clear()  # Clear pen drawings for the next frame render cycle
